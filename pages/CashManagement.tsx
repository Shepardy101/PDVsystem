...
const CashManagement: React.FC = () => {
  ...
  return (
    <div className="p-6 md:p-8 flex flex-col h-full overflow-hidden assemble-view bg-dark-950 bg-cyber-grid relative">
              ...
              <div className="flex items-center gap-1 border-b border-white/5 shrink-0 px-2 -mx-8 -mt-8 pt-4 bg-dark-950/40">
                 <button 
                  ...
                  className={`px-6 py-4 text-[10px] font-bold uppercase tracking-widest transition-all relative ${
                    historyModalTab === 'resumo' ? 'text-accent' : 'text-slate-500 hover:text-slate-300'
                  }`}
                 >
                 ...
                 <button 
                  ...
                  className={`px-6 py-4 text-[10px] font-bold uppercase tracking-widest transition-all relative ${
                    historyModalTab === 'movimentacoes' ? 'text-accent' : 'text-slate-500 hover:text-slate-300'
                  }`}
                 >
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="p-4 bg-white/2 rounded-xl border border-white/5">
                         ...
                         <p className="text-xs font-bold text-slate-300 flex items-center gap-2 uppercase tracking-tighter">
                           <Tag size={12} className="text-accent" /> {selectedTx.metadata?.category || 'Operacional'}
                         </p>
                      </div>
                   </div>
                </div>
      ...
      {/* Modais de Operação (Suprimento, Sangria, Pagamento, Fechamento) */}
      <Modal isOpen={isSuprimentoModalOpen} onClose={() => setIsSuprimentoModalOpen(false)} title="Injeção de Suprimento">
         <div className="space-y-6 animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setIsTxCategoryModalOpen(true)} className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl text-blue-400 hover:bg-blue-500/20 transition-all">
                    <FolderPlus size={18} />
                  </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
               <Button variant="secondary" className="py-4 uppercase text-[10px] font-bold tracking-widest" onClick={() => setIsSuprimentoModalOpen(false)}>Cancelar</Button>
               <Button className="py-4 uppercase text-[10px] font-bold tracking-widest shadow-blue-500/10 bg-blue-500/10 text-blue-400 border-blue-500/30 hover:bg-blue-500/20" icon={<Check size={18}/>} onClick={() => setIsSuprimentoModalOpen(false)}>Confirmar Entrada</Button>
            </div>
         </div>
      </Modal>
