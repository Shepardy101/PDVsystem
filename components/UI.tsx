...
export const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, footer, size = 'lg' }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
      ...
      <div className={`relative glass-panel rounded-2xl w-full ${sizeClasses[size]} shadow-glass animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh] overflow-hidden`}>
        ...
        <div className="p-4 sm:p-8 overflow-y-auto custom-scrollbar flex-1 min-h-0">
          {children}
        </div>
        {footer && (
          <div className="flex items-center justify-end gap-3 p-4 sm:p-6 border-t border-white/5 bg-white/2 shrink-0">
            {footer}
          </div>
        )}
      </div>
