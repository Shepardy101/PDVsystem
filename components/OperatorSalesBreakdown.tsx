...
import { User, ChevronDown, ChevronUp, DollarSign, ShoppingBag, Package, TrendingUp } from 'lucide-react';
...
const OperatorSalesBreakdown: React.FC<OperatorSalesBreakdownProps> = ({ sales, onTelemetry }) => {
  ...
  const operatorSalesData = useMemo(() => {
    ...
    return Array.from(operatorMap.values()).map(operator => ({
      ...operator,
      commission: operator.totalSales * COMMISSION_RATE,
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Análise de desempenho e cálculo de comissões</p>
          </div>
        </div>
                ...
                <div className="p-6 bg-dark-950/40">
                  ...
                  {operator.products.length > 0 ? (
                    <div className="space-y-2">
                      {/* Header da Tabela */}
                      <div className="grid grid-cols-12 gap-4 px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-slate-600 border-b border-white/5">
                      ...
                      {operator.products.map((product) => (
                        <div
                          ...
                          className="grid grid-cols-12 gap-4 px-4 py-3 bg-dark-900/40 rounded-lg hover:bg-dark-900/60 transition-colors"
                        >
                          ...
                          <div className="col-span-1 text-right text-sm font-bold text-emerald-400 font-num">
                            R$ {(product.totalRevenue * COMMISSION_RATE / 100).toFixed(2)}
                          </div>
                        </div>
                      ))}

                      {/* Totalizador */}
                      <div className="grid grid-cols-12 gap-4 px-4 py-3 mt-2 border-t border-white/5 bg-dark-950/60 rounded-lg">
                        ...
                        <div className="col-span-3 text-right text-base font-bold text-emerald-400 font-num">
                          R$ {(operator.commission / 100).toFixed(2)}
                        </div>
                      </div>
                    </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              A comissão padrão é de <strong className="text-blue-400">2%</strong> sobre o total de vendas. 
              Este valor pode ser configurado individualmente por operador ou por produto nas configurações do sistema.
            </p>
          </div>
        </div>
      </div>
