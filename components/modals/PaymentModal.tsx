...
const PaymentModal: React.FC<PaymentModalProps> = ({ isOpen, total, multiMode, setMultiMode, onClose, onFinalize }) => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (multiMode) {
        if (!isInputFocused) {
          if (focusStep === 'input' && e.key === 'Enter') {
            e.preventDefault();
            setFocusStep('select');
          } else if (focusStep === 'select' && e.key === 'Enter') {
            e.preventDefault();
            setFocusStep('add');
          } else if (focusStep === 'add' && e.key === 'Enter') {
            e.preventDefault();
            addBtnRef.current?.click();
          } else if (focusStep === 'finalize' && e.key === 'Enter') {
            e.preventDefault();
            finalizeBtnRef.current?.click();
          } 
        }
        if (e.key === '/' && multiMode === true) {
           e.preventDefault();
           setMultiMode(false);
           setTimeout(() => setFocusStep('input'), 0);
         }
        return;
      }
  ...
  }, [isOpen, multiMode, total, onFinalize, focusStep, setMultiMode]);
            ...
            <div className="space-y-4">
              <div className="flex gap-4 items-end">
                ...
                <Button
                  ...
                  onClick={e => {
                    // Evita duplo lançamento: só executa se não for disparado por Enter
                    if ((e as any).detail === 0) return; // se for disparado por Enter, ignora
